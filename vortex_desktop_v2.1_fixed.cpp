...
void DrawStatusPanel(Graphics& g, int sw) {
    ...
    if (g_musicPlaying) {
        for (int i = 0; i < 4; i++) {
            REAL h = (REAL)(4.0f + sinf((float)g_tick / 200.0f + i * 1.5f) * 6.0f);
            SolidBrush eqBr(VX::Magenta);
            g.FillRectangle(&eqBr, (REAL)(cx + i * 5), (REAL)(cy + 5) - h, 3.0f, h);
        }
    }
}
...
void DrawDesktopIcons(Graphics& g, int sw, int sh) {
    ...
    for (int i = 0; i < (int)g_desktopIcons.size(); i++) {
        ...
        RectF nameRc2((REAL)(x - 8), (REAL)(y + DESKTOP_ICON_SIZE), (REAL)(DESKTOP_ICON_SIZE + 16), (REAL)DESKTOP_ICON_TEXT_H);
...
void DrawWifiPanel(Graphics& g, int sw, int sh) {
    ...
    g.DrawString(L"[WiFi]  WiFi Networks", -1, &titleFont, titleRc, &lfmt, &titleBr);
...
void DrawContextMenu(Graphics& g) {
    ...
    for (int i = 0; i < (int)g_contextItems.size(); i++) {
        ...
        if (item.id == 0) {
            // Separator
            Pen sepPen(Color(30, 255, 255, 255), 1.0f);
            g.DrawLine(&sepPen, mx + 16, cy + sepH / 2, mx + menuW - 16, cy + sepH / 2);
            cy += sepH;
            continue;
        }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TIMER:
        ...
        if (wParam == 1) {
            ...
            for (auto& n : g_notifs) {
                if (!n.alive) continue;
                if (n.alpha < 1.0f) n.alpha = (std::min)(1.0f, n.alpha + 0.15f);
                if (n.offsetY < 0) n.offsetY = (std::min)(0.0f, n.offsetY + 8.0f);
