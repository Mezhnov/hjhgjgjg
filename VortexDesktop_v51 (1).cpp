/*
 * VORTEX Desktop Environment v5.1 - Ultra Smooth Fluent Design
 * Fixed compilation errors + Performance improvements + Premium Taskbar
 *
 * Fixes:
 * - FontStyleSemiBold replaced (not in GDI+) 
 * - Faster window opening with optimized spring physics
 * - Premium taskbar with depth, glow, reflections
 * - Smoother animations with better dt handling
 *
 * Controls:
...
// ============================================================================
//  FONT STYLE FIX - GDI+ doesn't have FontStyleSemiBold
// ============================================================================
// GDI+ only has: FontStyleRegular=0, FontStyleBold=1, FontStyleItalic=2,
// FontStyleBoldItalic=3, FontStyleUnderline=4, FontStyleStrikeout=8
// We emulate SemiBold by using Bold at slightly smaller size
#define FONT_SEMIBOLD FontStyleBold

// ============================================================================
//  SPRING PHYSICS + SMOOTH ANIMATION ENGINE (Optimized)
// ============================================================================
struct SpringValue {
    float current;
    float target;
...
const int TASKBAR_ICON_SIZE = 42;
...
// FASTER springs for snappier UI feel
...
struct TaskbarIconAnim {
    ...
    SpringValue glow;
    SpringValue bounce;
    TaskbarIconAnim() : hover(0, 320, 18), scale(1.0f, 400, 20), glow(0, 250, 16), bounce(0, 300, 18) {}
};
...
ExplorerWindow* CreateExplorerWindow(const std::wstring& path, const std::wstring& title) {
    ...
    win.w = 920;
    win.h = 620;
    // FASTER spring for instant-feel opening
...
// ============================================================================
//  TASKBAR - ULTIMATE PREMIUM DESIGN
//  Multi-layer glassmorphism + living glow + depth reflections
// ============================================================================
void DrawTaskbar(Graphics& g, int sw, int sh) {
    int barY = sh - TASKBAR_HEIGHT;

    // === LAYER 1: Deep base ===
    SolidBrush deepBase(Color(250, 14, 14, 16));
    g.FillRectangle(&deepBase, 0, barY, sw, TASKBAR_HEIGHT);

    // === LAYER 2: Gradient glass overlay ===
    LinearGradientBrush glassOverlay(Point(0, barY), Point(0, sh),
        Color(35, 80, 80, 90), Color(10, 20, 20, 22));
    g.FillRectangle(&glassOverlay, 0, barY, sw, TASKBAR_HEIGHT);

    // === LAYER 3: Horizontal gradient for depth ===
    LinearGradientBrush horzGlass(Point(0, barY), Point(sw, barY),
        Color(8, 40, 40, 50), Color(12, 60, 60, 70));
    g.FillRectangle(&horzGlass, 0, barY, sw, TASKBAR_HEIGHT);

    // === LAYER 4: Animated ambient glow strip (breathing) ===
    float breathe = 0.6f + 0.4f * sinf(g_time * 1.2f);
    int glowCenterX = sw / 2;
    for (int i = 0; i < 5; i++) {
        int gw = (int)(400 - i * 70);
        int gx = glowCenterX - gw / 2;
        BYTE ga = (BYTE)(breathe * (12 - i * 2));
        SolidBrush ambientGlow(Color(ga, 96, 205, 255));
        g.FillRectangle(&ambientGlow, gx, barY, gw, 3);
    }

    // === LAYER 5: Top edge — crisp glass edge with gradient ===
    {
        // Main bright line
        Pen topEdge(Color(55, 255, 255, 255), 1.0f);
        g.DrawLine(&topEdge, 0, barY, sw, barY);

        // Gradient glow under edge
        for (int i = 0; i < 3; i++) {
            SolidBrush edgeGlow(Color((BYTE)(20 - i * 6), 200, 210, 255));
            g.FillRectangle(&edgeGlow, 0, barY + 1 + i, sw, 1);
        }
    }

    // === LAYER 6: Subtle noise/texture simulation via dots ===
    // (skip for performance, implied by glass layers)
    ...
    // ======== START BUTTON — Premium floating design ========
    {
        int bx = startX;
        int by = iconY;
        ...
        // Hover/active background with multi-layer glow
        float bgIntensity = g_startMenuOpen ? 0.6f : ha * 0.45f;
        if (bgIntensity > 0.01f) {
            // Outer glow
            FillRoundRectSolid(g, bx - 4, by - 4, TASKBAR_ICON_SIZE + 8, TASKBAR_ICON_SIZE + 8, 12,
                Color((BYTE)(bgIntensity * 40), 96, 205, 255));
            // Inner bg
            FillRoundRectSolid(g, bx - 1, by - 1, TASKBAR_ICON_SIZE + 2, TASKBAR_ICON_SIZE + 2, 10,
                Color((BYTE)(bgIntensity * 200), 50, 55, 62));
        }

        // Windows 11 logo — crisp with perfect colors
        int logoSize = 16;
        ...
        Color logoC1, logoC2, logoC3, logoC4;
        if (g_startMenuOpen) {
            logoC1 = Color(255, 255, 255, 255);
            logoC2 = Color(255, 230, 240, 255);
            logoC3 = Color(255, 240, 230, 255);
            logoC4 = Color(255, 255, 245, 240);
        }
        else {
            float t = ha;
            BYTE r1 = (BYTE)(96 + t * 60);
            BYTE g1 = (BYTE)(205 + t * 30);
            BYTE b1 = 255;
            logoC1 = Color(255, r1, g1, b1);
            logoC2 = Color(255, (BYTE)(80 + t * 60), (BYTE)(190 + t * 40), 255);
            logoC3 = Color(255, (BYTE)(110 + t * 50), (BYTE)(200 + t * 30), 255);
            logoC4 = Color(255, r1, g1, b1);
        }

        // 4 rounded squares with individual colors for richness
        FillRoundRectSolid(g, lx, ly, sq, sq, 2, logoC1);
        FillRoundRectSolid(g, lx + sq + gap, ly, sq, sq, 2, logoC2);
        FillRoundRectSolid(g, lx, ly + sq + gap, sq, sq, 2, logoC3);
        FillRoundRectSolid(g, lx + sq + gap, ly + sq + gap, sq, sq, 2, logoC4);

        // Under-glow when active
        if (g_startMenuOpen || ha > 0.3f) {
            float glowI = g_startMenuOpen ? 0.5f : ha * 0.3f;
            int gcx = bx + TASKBAR_ICON_SIZE / 2;
            int gcy = by + TASKBAR_ICON_SIZE + 3;
            for (int i = 0; i < 4; i++) {
                int gr = 6 + i * 4;
                BYTE ga2 = (BYTE)(glowI * (30 - i * 7));
                SolidBrush gb(Color(ga2, 96, 205, 255));
                g.FillEllipse(&gb, gcx - gr, gcy - gr / 2, gr * 2, gr);
            }
        }
    }
    ...
    // ======== SEARCH BUTTON ========
    {
        ...
        Pen sp(Color(210, 190, 195, 210), 1.8f);
        sp.SetStartCap(LineCapRound);
        sp.SetEndCap(LineCapRound);
        int cx2 = bx + TASKBAR_ICON_SIZE / 2 - 2, cy2 = by + TASKBAR_ICON_SIZE / 2 - 2;
        g.DrawEllipse(&sp, cx2 - 7, cy2 - 7, 14, 14);
        g.DrawLine(&sp, cx2 + 5, cy2 + 5, cx2 + 8, cy2 + 8);
    }
    ...
    // ======== TASK VIEW BUTTON ========
    {
        ...
        Pen tp(Color(210, 190, 195, 210), 1.3f);
        tp.SetStartCap(LineCapRound);
        tp.SetEndCap(LineCapRound);
        // Two overlapping rectangles
        DrawRoundRect(g, bx + 10, by + 11, 11, 9, 2, Color(210, 190, 195, 210), 1.3f);
        DrawRoundRect(g, bx + 19, by + 20, 11, 9, 2, Color(140, 190, 195, 210), 1.3f);
    }

    // ======== APP ICONS — Premium floating pills ========
    ...
    for (int i = 0; i < iconCount; i++) {
        ...
        if (ha > 0.01f || g_taskApps[i].active) {
            float intensity = (std::max)(ha, g_taskApps[i].active ? 0.45f : 0.0f);

            // Outer glow ring
            FillRoundRectSolid(g, ix - 3, iconY - 3, TASKBAR_ICON_SIZE + 6, TASKBAR_ICON_SIZE + 6, 12,
                Color((BYTE)(intensity * 25), g_taskApps[i].glowColor.GetR(),
                    g_taskApps[i].glowColor.GetG(), g_taskApps[i].glowColor.GetB()));

            // Background pill
            FillRoundRectGradient(g, ix - 1, iconY - 1, TASKBAR_ICON_SIZE + 2, TASKBAR_ICON_SIZE + 2, 10,
                Color((BYTE)(intensity * 130), 62, 65, 72),
                Color((BYTE)(intensity * 100), 48, 50, 56));

            // Top edge highlight
            Pen topHL(Color((BYTE)(intensity * 40), 255, 255, 255), 0.7f);
            g.DrawLine(&topHL, ix + 5, iconY, ix + TASKBAR_ICON_SIZE - 5, iconY);
        }
        ...
        // Icon shadow
        FillRoundRectSolid(g, icoX + 1, icoY + 2, icoInner, icoInner, 8,
            Color(30, 0, 0, 0));

        Color c1 = g_taskApps[i].accentColor;
        Color c2(255,
            (BYTE)(std::max)(0, (int)c1.GetR() - 30),
            (BYTE)(std::max)(0, (int)c1.GetG() - 30),
            (BYTE)(std::max)(0, (int)c1.GetB() - 30));
        FillRoundRectGradient(g, icoX, icoY, icoInner, icoInner, 8, c1, c2);

        // Glass highlight on icon
        FillRoundRectSolid(g, icoX + 3, icoY + 1, icoInner - 6, icoInner / 3, 4,
        ...
        // === Running indicator — animated glowing pill ===
        if (g_taskApps[i].running) {
            int dotW = g_taskApps[i].active ? 22 : 6;
            int dotH = 3;
            int dotX = ix + (TASKBAR_ICON_SIZE - dotW) / 2;
            int dotY = iconY + TASKBAR_ICON_SIZE + 2;

            // Multi-layer glow under indicator
            for (int gl = 0; gl < 5; gl++) {
                int glW = dotW + gl * 6;
                int glX = ix + (TASKBAR_ICON_SIZE - glW) / 2;
                float glowPulse = 0.7f + 0.3f * sinf(g_time * 3.0f);
                BYTE glA = (BYTE)(glowPulse * (25 - gl * 5));
                SolidBrush glBr(Color(glA, 96, 205, 255));
                g.FillRectangle(&glBr, glX, dotY - gl, glW, dotH + gl * 2);
            }

            // Main indicator pill
            FillRoundRectGradient(g, dotX, dotY, dotW, dotH, 1,
                Color(255, 140, 220, 255), Color(255, 60, 180, 255), false);
        }
        ...
        if (ha > 0.5f) {
            ...
            for (int gl = 0; gl < 3; gl++) {
                int gr = 4 + gl * 3;
                BYTE ga2 = (BYTE)(ha * (12 - gl * 4));
                SolidBrush topGlow(Color(ga2, g_taskApps[i].glowColor.GetR(),
                    g_taskApps[i].glowColor.GetG(), g_taskApps[i].glowColor.GetB()));
                g.FillEllipse(&topGlow, gcx - gr, gcy - gr / 2, gr * 2, gr);
            }
        }
    ...
    // ======== SYSTEM TRAY — Premium pill with depth ========
    int trayX = sw - 250;

    // Tray background — frosted pill
    FillRoundRectGradient(g, trayX - 8, barY + 8, 244, TASKBAR_HEIGHT - 16, 10,
        Color(30, 80, 80, 90), Color(20, 40, 40, 45));
    DrawRoundRect(g, trayX - 8, barY + 8, 244, TASKBAR_HEIGHT - 16, 10,
        Color(20, 255, 255, 255), 0.5f);

    // Hidden icons chevron
    {
        int cx2 = trayX + 6;
        int cy2 = barY + TASKBAR_HEIGHT / 2;
        Pen chev(Color(90, 190, 195, 210), 1.5f);
        chev.SetStartCap(LineCapRound);
        chev.SetEndCap(LineCapRound);
        g.DrawLine(&chev, cx2 - 2, cy2 - 4, cx2 + 2, cy2);
        g.DrawLine(&chev, cx2 + 2, cy2, cx2 - 2, cy2 + 4);
    }

    // ---- WiFi Icon — Animated waves with glow ----
    {
        int wbx = trayX + 28;
        int wby = barY + (TASKBAR_HEIGHT - 24) / 2;
        bool wifiHov = (g_hoveredTaskbarIcon == -20);

        if (wifiHov || g_wifiPanelOpen) {
            FillRoundRectSolid(g, wbx - 6, wby - 6, 36, 36, 8, Color(60, 96, 205, 255));
            DrawRoundRect(g, wbx - 6, wby - 6, 36, 36, 8, Color(30, 96, 205, 255), 0.5f);
        }

        int wcx = wbx + 12;
        int wcy = wby + 19;

        // WiFi waves — smooth arcs with animated pulse
        for (int arc = 0; arc < 3; arc++) {
            int r = 5 + arc * 6;
            float wavePhase = g_time * 2.0f + arc * 0.6f;
            float waveAlpha = 0.85f - arc * 0.12f;
            float pulse = 1.0f + 0.04f * sinf(wavePhase);
            int pr = (int)(r * pulse);

            Pen wPen(Color((BYTE)(220 * waveAlpha), 180, 210, 255), 2.2f);
        ...
        for (int i = 0; i < 3; i++) {
            int r = 5 - i;
            SolidBrush dotGlow(Color((BYTE)(20 + i * 10), 96, 205, 255));
            g.FillEllipse(&dotGlow, wcx - r, wcy - r, r * 2, r * 2);
        }
        SolidBrush dotBr(Color(250, 230, 240, 255));
        g.FillEllipse(&dotBr, wcx - 2, wcy - 2, 5, 5);
    }

    // ---- Volume Icon — Premium speaker with arcs ----
    {
        int vx = trayX + 76;
        int vy = barY + (TASKBAR_HEIGHT - 22) / 2;

        Pen vPen(Color(210, 200, 210, 230), 1.6f);
        ...
        // Sound waves with gradient opacity
        for (int w = 0; w < 2; w++) {
            int wr = 5 + w * 5;
            float waveA = 0.75f - w * 0.2f;
            Pen wp(Color((BYTE)(210 * waveA), 200, 210, 230), 1.4f);
            wp.SetStartCap(LineCapRound);
            wp.SetEndCap(LineCapRound);
            g.DrawArc(&wp, vx + 11, vy + 11 - wr, wr * 2, wr * 2, -55, 110);
        }
    }

    // ---- Battery Icon — Modern gradient fill ----
    {
        int bx = trayX + 114;
        int by = barY + (TASKBAR_HEIGHT - 16) / 2;

        // Battery outline with rounded corners
        DrawRoundRect(g, bx, by, 26, 16, 3, Color(200, 200, 210, 230), 1.3f);

        // Battery tip
        FillRoundRectSolid(g, bx + 26, by + 4, 3, 8, 1, Color(200, 200, 210, 230));

        // Fill with gradient — 82%
        float batteryLevel = 0.82f;
        int fillW = (int)(20 * batteryLevel);
        Color battC1(255, 108, 220, 100);
        Color battC2(255, 72, 185, 65);
        FillRoundRectGradient(g, bx + 3, by + 3, fillW, 10, 2, battC1, battC2, false);

        // Glass shine on battery
        FillRoundRectSolid(g, bx + 3, by + 3, fillW, 4, 1, Color(25, 255, 255, 255));
    }

    // ---- Clock — Beautiful typography with depth ----
    ...
    // Clock text shadow
    SolidBrush clockShadow(Color(60, 0, 0, 0));
    RectF clockShadowRc((REAL)(sw - 107), (REAL)(barY + 1), 96.0f, (REAL)(TASKBAR_HEIGHT / 2 + 2));
    g.DrawString(timeStr, -1, &clockFont, clockShadowRc, &rfmt, &clockShadow);

    // Clock text
    SolidBrush clockBr(Color(255, 250, 250, 255));
    RectF clockRc((REAL)(sw - 108), (REAL)barY, 96.0f, (REAL)(TASKBAR_HEIGHT / 2 + 2));
    g.DrawString(timeStr, -1, &clockFont, clockRc, &rfmt, &clockBr);

    SolidBrush dateBr(Color(185, 190, 195, 210));
    RectF dateRc((REAL)(sw - 108), (REAL)(barY + TASKBAR_HEIGHT / 2 - 2), 96.0f, (REAL)(TASKBAR_HEIGHT / 2));
    g.DrawString(dateStr, -1, &dateFont, dateRc, &rfmt, &dateBr);

    // Notification indicator with animated glow
    if (!g_notifs.empty()) {
        int nix = sw - 14, niy = barY + TASKBAR_HEIGHT / 2 - 3;
        float pulse = 0.5f + 0.5f * sinf(g_time * 4.0f);
        DrawGlowEffect(g, nix + 3, niy + 3, 4, W11::AccentLight, 0.4f + pulse * 0.3f);
        SolidBrush notifDot(W11::AccentLight);
        g.FillEllipse(&notifDot, nix, niy, 7, 7);
    }

    // === Bottom reflection line (subtle) ===
    LinearGradientBrush bottomRef(Point(sw / 2 - 300, sh - 1), Point(sw / 2 + 300, sh - 1),
        Color(0, 255, 255, 255), Color(15, 255, 255, 255));
    Pen bottomLine(Color(8, 255, 255, 255), 0.5f);
    g.DrawLine(&bottomLine, 0, sh - 1, sw, sh - 1);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ...
        SetTimer(hWnd, 2, 7, NULL); // ~144fps update timer
    ...
    case WM_TIMER:
        ...
        if (wParam == 1) {
            ...
            // Precise delta time for smooth physics
            LARGE_INTEGER now;
            QueryPerformanceCounter(&now);
            float dt = (float)(now.QuadPart - g_lastFrame.QuadPart) / (float)g_perfFreq.QuadPart;
            g_lastFrame = now;
            if (dt > 0.05f) dt = 0.016f; // Cap to prevent jumps
            if (dt < 0.001f) dt = 0.007f;

            // Spring physics updates with precise dt
    ...
    case WM_MOUSEMOVE: {
        ...
        if (my >= barY) {
            ...
            int searchBtnX = startX + (TASKBAR_ICON_SIZE + 8);
            ...
            int tvBtnX = startX + 2 * (TASKBAR_ICON_SIZE + 8);
            ...
            int trayX = sw - 250;
            RECT wifiRect; SetRect(&wifiRect, trayX + 22, barY, trayX + 62, barY + TASKBAR_HEIGHT);
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int nShow) {
    ...
    wc.style = CS_DBLCLKS;
    wc.lpszClassName = L"VORTEX_Desktop_v51";
    RegisterClassExW(&wc);

    int sw = ScreenW(), sh = ScreenH();
    g_hWnd = CreateWindowExW(
        WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
        L"VORTEX_Desktop_v51", L"VORTEX Desktop v5.1",
